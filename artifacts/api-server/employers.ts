...
import {
  ...
  ListTradeInterestsQueryParams,
  ListTradeInterestsResponse,
  AddTradeInterestBody,
  RemoveTradeInterestParams,
  RemoveTradeInterestQueryParams,
} from "@workspace/api-zod";

const router: IRouter = Router();

router.get("/profile", async (req, res): Promise<void> => {
  const query = GetProfileQueryParams.safeParse(req.query);
  if (!query.success) {
...
router.post("/profile", async (req, res): Promise<void> => {
  ...
  if (!user) {
    ...
    [user] = await db
      .update(users)
      .set({ email, firstName: firstName ?? user.firstName, lastName: lastName ?? user.lastName, updatedAt: new Date() })
  ...
  if (!existingProfile) {
    [profile] = await db
      ...
      .values({
        userId: user.id,
        graduationStatus: (profileData.graduationStatus as any) ?? "current_senior",
    ...
    [profile] = await db
      ...
      .set({
        graduationStatus: (profileData.graduationStatus as any) ?? existingProfile.graduationStatus,
        ...
        interestedInApprenticeship: profileData.interestedInApprenticeship ?? existingProfile.interestedInApprenticeship,
...
router.get("/profile/trade-interests", async (req, res): Promise<void> => {
  const query = ListTradeInterestsQueryParams.safeParse(req.query);
  if (!query.success) {
    res.status(400).json({ error: query.error.message });
    return;
  }
  ...
  res.json(ListTradeInterestsResponse.parse(rows));
});

router.post("/profile/trade-interests", async (req, res): Promise<void> => {
  const parsed = AddTradeInterestBody.safeParse(req.body);
  if (!parsed.success) {
    res.status(400).json({ error: parsed.error.message });
    return;
  }
...
router.delete("/profile/trade-interests/:tradeId", async (req, res): Promise<void> => {
  const params = RemoveTradeInterestParams.safeParse(req.params);
  if (!params.success) {
    res.status(400).json({ error: params.error.message });
    return;
  }

  const query = RemoveTradeInterestQueryParams.safeParse(req.query);
  if (!query.success) {
    res.status(400).json({ error: query.error.message });
    return;
  }
